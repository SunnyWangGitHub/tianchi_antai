...
# 0.0 1583 0.7432692816678316  ==> stacking!
...
# 0.02 698   0.7433305505335608 ==> stacking!
# 0.03 565   0.7432965713923161

# 0.02  0.7433305505335608 + cate_cols 0.7434669703043668
# for t in [0,0.008]:
# for t in [0.02]:
for t in [0,0.008,0.02]:
    print('!!!!!!!!!!!!!!!!!!!!!, ',t)
    new_feat_imp_df = pd.read_csv('../sub/cat/0.7432692816678316/feat_cat_baseline.csv')
    ...
    for seed in seeds:
        ...
        for i, (trn_idx, val_idx) in enumerate(skf.split(train, train['isDefault'])):
            ...
            old_cate_cols = ['applicationType', 'employmentLength', 'employmentTitle', 'grade', 'homeOwnership', 'initialListStatus',
                            'postCode', 'purpose', 'regionCode', 'subGrade', 'title', 'verificationStatus']

            cate_cols = [fea for fea in old_cate_cols if fea in cols]
            ...
            oof[val_idx] = cat_model.predict_proba(val_x)[:, 1]
            test['isDefault'] += cat_model.predict_proba(test[cols])[:, 1] / skf.n_splits / len(seeds)
            test_sqrt['isDefault'] += cat_model.predict_proba(test[cols])[:, 1]**0.5 / skf.n_splits / len(seeds)
            
            # predictions[:,i] += cat_model.predict_proba(test[cols])[:, 1]/ skf.n_splits
    ...
    save_path = '../sub/cat/'+str(result_aucs)
    ...
    submit.to_csv(os.path.join(save_path,'baseline_cat_auc_{}.csv'.format(result_aucs)), index = False)

    ##################################
    submit['isDefault'] = test_sqrt['isDefault']

    submit.to_csv(os.path.join(save_path,'baseline_cat_auc_{}_sqrt.csv'.format(result_aucs)), index = False)


    ##################################

    df_importance.to_csv(os.path.join(save_path,'feat_cat_baseline.csv'), index = False)

    np.save(os.path.join(save_path,'baseline_cat_auc_{}.npy'.format(result_aucs)),oof)



    # #################################
    # print("Rank averaging on", len(seeds), "seeds predictions(base on sqrt)")
    ...
    # submit.to_csv(os.path.join(save_path,'baseline_cat_auc_{}_rank.csv'.format(result_aucs)), index = False)
