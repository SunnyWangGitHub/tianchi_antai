...
# # for t in [0.0001,0.00015,0.0002,0.00025,0.0003,0.00035,0.0004,0.00045,0.0005]:
# #     print('!!!!!!!!!!!!!!!!!!!!!, ',t)
...
# #             #                         # max_depth=int(5.031476228084266),
# #             #                         max_depth = 6
# #             #                         learning_rate=0.05,
# #             #                         colsample_bytree =  0.8071162699206316,
# #             #                         gamma =  1.6897867231675445,
# #             #                         max_delta_step = int(9.83455709500259),
# #             #                         min_child_weight = 19.08915534938428,
# #             #                         subsample = 0.7719247898383148,
...
# #             # #                           tree_method='gpu_hist',
# #             #                         random_state=seed)
# #             # 0.7402604356412531  ==> 0.7398722181425847 贝叶斯不行

# #             xgb_model =  model.fit(trn_x,
# #                                     trn_y,
...
# #             oof[val_idx] = xgb_model.predict_proba(val_x)[:, 1]
# #             test['isDefault'] += xgb_model.predict_proba(test[cols])[:, 1] / skf.n_splits / len(seeds)
# #             test_sqrt['isDefault'] += xgb_model.predict_proba(test[cols])[:, 1]**0.5 / skf.n_splits / len(seeds)
...
# #     save_path = '../sub/xgb/'+str(result_aucs)
...
# #     submit.to_csv(os.path.join(save_path,'baseline_xgb_auc_{}.csv'.format(result_aucs)), index = False)

# #     ##################################
# #     submit['isDefault'] = test_sqrt['isDefault']

# #     submit.to_csv(os.path.join(save_path,'baseline_xgb_auc_{}_sqrt.csv'.format(result_aucs)), index = False)


# #     ##################################

# #     df_importance.to_csv(os.path.join(save_path,'feat_xgb_baseline.csv'), index = False)

# #     np.save(os.path.join(save_path,'baseline_xgb_auc_{}.npy'.format(result_aucs)),oof)

# old_cols = [col for col in train.columns if col not in ['id','isDefault','index']]
# new_feat_imp_df = pd.read_csv('../sub/xgb/0.7427262249215246/feat_xgb_baseline.csv')
# new_feat_imp_df = new_feat_imp_df[new_feat_imp_df['importance']>=0.0001]
...
test_stacking = pd.read_csv('./10sub_1_stacking_BR_auc_0.7457619493976538.csv ')
...
def process(x):
    if x <=0.5:
        ...
        return 1
test['isDefault'] = test_stacking['isDefault'].apply(process)

test['isDefault'].value_counts()
...
for seed in seeds:
    ...
    for i, (trn_idx, val_idx) in enumerate(skf.split(train, train['isDefault'])):
        ...
        # predictions[:,i] += xgb_model.predict_proba(test[cols])[:, 1]/ skf.n_splits
...
# #################################
# print("Rank averaging on", len(seeds), "seeds predictions")
...
# rank_predictions /= len(seeds)
# submit['isDefault'] = rank_predictions
# submit.to_csv(os.path.join(save_path,'baseline_xgb_auc_{}_rank.csv'.format(result_aucs)), index = False)
